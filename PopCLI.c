...
/************************************************************************/
struct InputEvent *myhandler(struct InputEvent *ev, GLOBAL_DATA *gptr) {
  struct InputEvent *ep, *laste;

  static char buf[256];

  mysprintf(buf, "%ld %ld(%ld) %ld\n",
            (unsigned int)ev->ie_Class == IECLASS_RAWKEY,
            (unsigned int) ev->ie_Code, (unsigned int) gptr->key,
            (unsigned int) ev->ie_Qualifier & IEQUALIFIER_RCOMMAND);
  gptr->dmsg = buf;
  ...
  for (ep = ev, laste = NULL; ep != NULL; ep = ep->ie_NextEvent) {
    if ((ep->ie_Class == IECLASS_RAWKEY)    &&
        (ep->ie_Code  == gptr->key)
         && (ep->ie_Qualifier & IEQUALIFIER_RCOMMAND)
        ) {
...
/************************************************************************/
void QueueTimer(struct timerequest *tr, ULONG seconds) {
...
/*
  void _main(char *cmd, int cmdlen) {
  _Backstdout = Output();
...
int _main(char *cmd, int cmdlen) {
  ...
  _Backstdout = stdout = Output();
  //Write(stdout, cmd, cmdlen);
  strcpy(cmdstr, DEFCMD);
  //Write(_Backstdout, cmdstr, strlen(cmdstr));
  ...
  for(;;) {        /* FOREVER */
    sig = Wait( global.creatclisig | global.unblanksig | timersig | SIGBREAKF_CTRL_C );
    //Write(stdout, cmdstr, strlen(cmdstr));
    //Write(stdout, global.dmsg, strlen(global.dmsg));
    ...
    if ((msg = (struct OURMSG *)GetMsg(port)) != NULL) {
      //Write(stdout, "holy shit\n", 10);
      if (msg->cmd[0])
        strcpy(cmdstr, msg->cmd);
      global.key = msg->key;
      timeout    = msg->interval;
      FreeMem((void *)msg, msg->msgpart.mn_Length);
...
void DeletePort(struct MsgPort *port) {
  ...
  FreeMem((void *)port, sizeof(struct MsgPort));
}
...
void DeleteIOReq(struct IOStdReq *ioReq) {
  ...
  FreeMem((void *)ioReq, ioReq->io_Message.mn_Length);
}
